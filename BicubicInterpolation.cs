...
namespace PhotoCorrection {
	class BicubicInterpolation {
        ...
        private static void CLAMP(ref int value, int min, int max) {
            /* Приведение значения к мин./макс. значения при выходах за границы */
            if (value < min) { value = min; } else if (value > max) { value = max; }
		}

        private static Color GetPixelClamped(Bitmap img, int x, int y) {
            /* Возвращает значение пикселы в точках x, y
             * предварительно проверив, что точки x, y - принадлежат изображению
             */
